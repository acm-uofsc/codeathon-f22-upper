...
if case_num == 0:
    ...
    highest_possible_iq = 10_000
    number_of_history_people = random.randint(2, 10)
    num_visitors = random.randint(2, 10)

    if case_num > 10:
        num_visitors = random.randint(2, 100_000)
        number_of_history_people = random.randint(100_000, 500_000)
        highest_possible_iq = 1_000_000_000
    if case_num > 25:
        number_of_history_people = 400_000
        repeat = 10000
        history_people = [random.randint(0, highest_possible_iq)
                          for i in range(number_of_history_people // repeat)]*repeat
        num_visitors = 100_000
    else:
        history_people = [random.randint(0, highest_possible_iq)
                          for i in range(number_of_history_people)]
    history_people.sort()
    visitors = [random.choice(history_people) + random.randint(0, 2)
